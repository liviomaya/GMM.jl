...
@testset "Multivariate Regression" begin
    ...
    @testset "Solution Precision: Test 1" begin
        ...
        n = 10000
        x = randn(n, 2)
        z = [x + 0.1 * randn(n, 2) (x[:, 1] + 0.1 * randn(n))]
        e = 0.1 * randn(n, 2)
        y = zeros(n, 2)
        y[:, 1] = 1.0 .+ 2.0 .* x[:, 1] .+ 3.0 .* x[:, 2] .+ e[:, 1]
        y[:, 2] = -1.0 .- 2.0 .* x[:, 1] .- 3.0 .* x[:, 2] .+ e[:, 2]
        ...
        @test abs(ols_reg.intercept[1] - 1.0) < 0.1
        @test abs(ols_reg.coef[1, 1] - 2.0) < 0.1
        @test abs(ols_reg.coef[1, 2] - 3.0) < 0.1
        @test abs(ols_reg.intercept[2] - (-1.0)) < 0.1
        @test abs(ols_reg.coef[2, 1] - (-2.0)) < 0.1
        @test abs(ols_reg.coef[2, 2] - (-3.0)) < 0.1
        @test abs(iv_reg.intercept[1] - 1.0) < 0.1
        @test abs(iv_reg.coef[1, 1] - 2.0) < 0.1
        @test abs(iv_reg.coef[1, 2] - 3.0) < 0.1
        @test abs(iv_reg.intercept[2] - (-1.0)) < 0.1
        @test abs(iv_reg.coef[2, 1] - (-2.0)) < 0.1
        @test abs(iv_reg.coef[2, 2] - (-3.0)) < 0.1

        # Check that the estimated coefficients are statistically significant
        @test maximum(abs, ols_reg.tStat) > 2
        @test maximum(abs, iv_reg.tStat) > 2

        # Check that the estimated residual variance is close to true value
        @test maximum(abs, sqrt.(diag(ols_reg.resCov)) .- 0.1) < 0.02
        @test maximum(abs, ols_reg.resCov[1, 2]) < 0.02
        @test maximum(abs, sqrt.(diag(iv_reg.resCov)) .- 0.1) < 0.02
        @test maximum(abs, iv_reg.resCov[1, 2]) < 0.02
    ...
    @testset "Rank and Order Conditions" begin
        ...
        @test_throws ArgumentError regIV(zeros(n, 2), x, z[:, 1])
        @test_throws ArgumentError regIV(zeros(n, 2), x, z)
    end
end